...
export const StaysTableRow = ({ data }: Props) => {
  ...
  return (
    <TableRow>
      <div className="w-[40%] md:w-[25%] z-20">
        {clientInStay.map(({ client }, ix) => (
          <p
            key={"name_row_history_stay_" + client.numberDocument + ix}
      ...
      <div className="w-[30%] md:w-[17.5%] flex flex-col gap-0.5">
        ...
        <div className="flex gap-1 items-center">
          <FaWalking className="text-out-client px-1.5 py-1 rounded-md bg-out-client/20 size-6 -scale-x-100 z-10" />
      ...
      <div className="md:w-[10%] hidden md:flex justify-center ">
        {Array.from({ length: stars || 0 }, (_, ix) => (
          <FaStar key={"star_" + ix} className="text-stars" />
        ))}
      </div>
