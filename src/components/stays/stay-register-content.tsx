...
import { FaDoorClosed, FaPen, FaSave, FaUsers } from 'react-icons/fa'
...
import { AddClientStay } from './add-client-stay'
...
export const StayRegisterContent = ({ rooms }:Props) => {
  ...
  const {carPlate,clientInStay,dateStart,origin,reason,id,pays,comments} = currentData
  ...
  return (
    <div className=" w-full flex flex-col gap-2">
      ...
      <div className="w-full flex px-3 md:px-5 py-2 items-center bg-white-01 rounded-t-2xl">
        ...
        <DeleteStay room={currentRoom} stayId={id} dateStart={dateStart}/>
      </div>
      ...
      <div className="w-full flex px-3 md:px-5 py-2 items-center bg-white-01 rounded-t-2xl mt-4">
        <div className="font-bold text-lg md:text-xl 2xl:text-2xl text-gray-05 flex items-center gap-2 py-2 w-full uppercase">
          <FaUsers className='size-7'/> 
          <p>Clientes</p>
        </div>

        <AddClientStay clientList={clientInStay} currentRoom={currentRoom} stayId={id}/>
      </div>
      ...
      <CloseStayForm stayId={id} pays={pays} room={currentRoom} stayComments={stayComments}/>
    </div>
  )
}
