...
export const NewClientForm = ({ countries, showButton = true, cancelDialog }: Props) => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "typeDocument") {
      if (value === "DNI") setNewClientData((prev) => ({ ...prev, numberDocument: prev.numberDocument.slice(0, 8) }));
      else if (value === "Carnet_Extranjeria")
        setNewClientData((prev) => ({ ...prev, numberDocument: prev.numberDocument.slice(0, 12) }));
    }
  ...
  const handleClick = async () => {
    ...
    if (phone !== "" && phone.length < 9) return stSetStaticMsg("El telefono debe ser de al menos 9 caracteres");

    if (newFirstName.length < 3 || newLastName.length < 3)
      return stSetStaticMsg("El Nombre/Apellido debe de ser de mas de 2 caracteres");

    if (numberDocument.length < 6) return stSetStaticMsg("El Documento debe de ser de mas de 6 caracteres");

    if (typeDocument === "DNI" && numberDocument.length !== 8) return stSetStaticMsg("El DNI debe tener 8 numeros");
    ...
    <>
      ...
      <CenterDialog id={dialogClient}>
        <DialogContent maxWRem={40}>
          ...
          <DialogFooterSave id={dialogClient} saveClick={handleClick} cancelDialog={cancelDialog} />
        </DialogContent>
      </CenterDialog>
    </>
  );
