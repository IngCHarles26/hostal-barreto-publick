...
export const SAgetClientByDocument = async (typeDocument: TypeDocuments, numberDocument: string) => {
  try {
    ...
    if (clientInStay)
      return {
        success: false,
        message: `El cliente ${name} esta actualmente en la habitacion ${clientInStay.room.number}`,
      };
...
export const SAEditClientDocument = async (clientId: string, numberDocument: string) => {
  try {
    await getUserInfo();
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) return { success: false, message: "El cliente no existe" };

    await prisma.client.update({ where: { id: clientId }, data: { numberDocument } });
...
export const SAEditClientBorn = async (clientId: string, born: Date) => {
  try {
    await getUserInfo();
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) return { success: false, message: "El cliente no existe" };

    await prisma.client.update({ where: { id: clientId }, data: { born } });

    return { success: true, message: "Nacimiento actualizado correctamente" };
  } catch (err) {
    consoleError(err);
    return { success: false, message: "No se pudo realizar la busqueda" };
  }
...
export const SAEditClientAddress = async (clientId: string, address: string) => {
  try {
    await getUserInfo();
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) return { success: false, message: "El cliente no existe" };

    await prisma.client.update({ where: { id: clientId }, data: { address } });

    return { success: true, message: "Direccion actualizada correctamente" };
  } catch (err) {
    consoleError(err);
...
export const SAEditClientPhone = async (clientId: string, phone: string) => {
  try {
    await getUserInfo();
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) return { success: false, message: "El cliente no existe" };
    ...
    return { success: true, message: "Telefono actualizado correctamente" };
  } catch (err) {
    consoleError(err);
    return { success: false, message: "No se pudo actualizar el telefono" };
  }
};

export const SAEditClientName = async (clientId: string, firstName: string, lastName: string) => {
  try {
    await getUserInfo();
    const client = await prisma.client.findUnique({ where: { id: clientId } });
    if (!client) return { success: false, message: "El cliente no existe" };

    await prisma.client.update({ where: { id: clientId }, data: { firstName, lastName } });
