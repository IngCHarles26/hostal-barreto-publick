  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.stay.create({
        data: { ...data, userId: user.id, paidUntil: dateStart },
        include: {
          clientInStay: true,
        },
      });
...
export const SAcloseStay = async (data: CloseStayInterface) => {
  ...
  try {
    ...
    await prisma.$transaction(
      async (tx) => {
        const dataFetch: TypeFetchGoogleFolder = await (
          await fetch(API_GOOGLE_FOLDERS + stayId, {
...
export const SAaddClientsToActiveStay = async (stayId: number, clientIds: string[]) => {
  try {
    await getUserInfo();
    await prisma.clientInStay.createMany({
      data: clientIds.map((clientId) => ({ clientId, stayId })),
    });
